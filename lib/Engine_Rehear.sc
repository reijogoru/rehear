...
synthSampler =  Synth("Rehear",target:context.server);
    ...
    buffer =	Buffer.cueSoundFile(context.server,msg[1],0,2,bufferSize: 4096);
...
OSCFunc({ arg msg;
    ...
    var index = msg[3];
     ...
     {NetAddr("127.0.0.1", 10111).sendMsg("position",position.asInteger,"duration",c.duration);}
